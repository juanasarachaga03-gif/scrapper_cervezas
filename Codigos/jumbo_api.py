...
# --- Constantes de JUMBO (CORREGIDAS) ---

BUSQUEDAS_JUMBO = [
    # cerveza principal (muchas páginas)
    *(f"https://www.jumbo.com.ar/api/catalog_system/pub/products/search/cerveza?_from={i}&_to={i+49}" for i in range(0, 600, 50)),

    # variantes clave (muy importante)
    *(f"https://www.jumbo.com.ar/api/catalog_system/pub/products/search/beer?_from={i}&_to={i+49}" for i in range(0, 200, 50)),
    *(f"https://www.jumbo.com.ar/api/catalog_system/pub/products/search/lager?_from={i}&_to={i+49}" for i in range(0, 200, 50)),
    *(f"https://www.jumbo.com.ar/api/catalog_system/pub/products/search/ipa?_from={i}&_to={i+49}" for i in range(0, 200, 50)),
...
URL_PRECIO_SKU = "https://www.jumbo.com.ar/api/catalog_system/pub/products/search?fq=skuId:"
URL_PROMO = "https://www.jumbo.com.ar/_v/search-promotions"
...
def fetch_single_item_catalog(url, status_box):
    ...
    try:
        ...
        status_box.warning(f"  [Jumbo Catalog] URL fallida: {url}. Error: {e}")
        return []

def fetch_jumbo_items(status_box):
    """
    Paso 1: Obtiene la lista maestra de todos los Item IDs de Jumbo en paralelo.
    """
    status_box.write(f"→ Iniciando {len(BUSQUEDAS_JUMBO)} búsquedas paralelas de catálogo maestro...")
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS_CATALOGO) as executor:
        ...
        futures = {executor.submit(fetch_single_item_catalog, url, status_box): url for url in BUSQUEDAS_JUMBO}
    ...
    status_box.write(f"→ Recolectados {total_items} Item IDs únicos para Jumbo (Tiempo: {end_time - start_time:.2f}s)")
...
def fetch_single_item_data(item, codigo_suc_vtex, codigo_ccx):
    ...
    try:
        ...
        try:
            ...
            # print(f"[Debug Jumbo] SKU {item_id}: Error al parsear JSON de precio. Error: {e}. Respuesta: {response_precio.text[:100]}...")
            return None
        ...
        if stock > 0:
            ...
            # print(f"[Debug Jumbo] SKU {item_id}: Stock 0. (Cookie: {codigo_suc_vtex})")
            return None # Producto sin stock
            
    except requests.exceptions.RequestException as e_precio:
        # print(f"[Debug Jumbo] SKU {item_id}: Falló la llamada de PRECIO. Error: {e_precio}")
        return None # Falla la llamada de precio

def fetch_and_process_jumbo_prices_parallel(master_items_jumbo, nombre_suc, provincia, codigo_suc_vtex, codigo_ccx, status_box, progress_bar, current_step, total_steps):
    """
    Paso 2: Obtiene precios y promos para todos los items de la lista maestra, en paralelo.
    """
    
    total_items = len(master_items_jumbo)
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS_PRECIO) as executor:
        ...
        futures = {
            ...
            for item in master_items_jumbo
        }
    ...
    status_box.write(f"→ Finalizado Jumbo para {nombre_suc}. Se obtuvieron {len(lista_productos_sucursal)} productos con stock.")
